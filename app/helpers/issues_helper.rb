...
module IssuesHelper
  ...
  def render_descendants_tree(issue)
    ...
    if User.current.allowed_to?(:assigned_to, @project)
      if User.current.allowed_to?(:done_ratio, @project)
  ...
  def render_custom_fields_rows(issue)
    ...
    ordered_values.compact.each do |value|
      #      if value.custom_field.id!=59 && excluded_rol?(value.custom_field)
  ...
  def excluded_rol?(custom_field)
    validator=true   
    igualdades=0
    roles = User.current.roles_for_project(@project);
    custom_field.roles.each { |r|
      roles.each { |cr|
        (igualdades+=1) if (cr.id==r.id)
      }
    }
    (validator=false) if (igualdades == roles.count)
    validator
  end

  def role_allowed_by_custom_field?(object,value)
    @member = Member.find_by_user_id_and_project_id(User.current.id, @project)
    @memberRoles = MemberRole.find_all_by_member_id(@member)
    permission=nil
    bool=0
    @memberRoles.each do |memberRole|
      rol = Role.find_by_id(memberRole.role_id)
      @cfir = nil
      @cfir = object.find_by_role_id_and_custom_field_id(rol,value.custom_field)
      if @cfir.blank?
        permission = true
        bool=1
      else
    ...
    # Sin este if basta con que tenga un falso para que no se muestre
    if bool==1
      permission = true
    end
    permission
  ...
  def show_detail(detail, no_html=false, options={})
    ...
    when 'attr'
      ...
      when 'route_id'
        ...
        (old_value = User.find(detail.old_value) unless detail.value.blank?) if detail.old_value
      end
    when 'cf'
      ...
      if custom_field
        ...
        if detail.value.instance_of?(Date)
          value = format_date(detail.value.to_date) if detail.value
          old_value = format_date(detail.old_value.to_date) if detail.old_value and !detail.old_value.blank?
        else
