...
module CustomFieldsHelper
  ...
  # Return custom checkbox tag with its label tag
  def custom_check_box_tag_with_label(name, custom_value)
    custom_field_label_tag(name, custom_value) + custom_check_box_tag(name, custom_value)
  end

  def custom_check_box_tag_with_label_2(name, custom_value)
    custom_field_label_tag(name, custom_value) + custom_check_box_tag_2(name, custom_value)
  end

  def custom_check_box_tag_2(name, custom_value)
    custom_field = custom_value.custom_field
    field_name = "#{name}[custom_field_values][#{custom_field.id}]"
    @field_id = "#{name}_custom_field_values_#{custom_field.id}"
    hidden_field_tag(field_name, '0') + check_box_tag(field_name, '1',
      true,
      {:id => "#{@field_id}_box"}) # :id => field_id)
  end

  def custom_check_box_disabled_tag(name, custom_value)
    custom_field = custom_value.custom_field
    field_name = "#{name}[custom_field_values][#{custom_field.id}]"
    @field_id = "#{name}_custom_field_values_#{custom_field.id}"
    custom_field_label_tag(name, custom_value) + check_box_tag(field_name, '1',
      true, # custom_value.true?,
      {:id => "#{@field_id}_box", :disabled=> true}) # :id => field_id)
  end

  def submit_tag_helper(value = "Save changes", options = {})
    options.stringify_keys!

    if confirm = options.delete("confirm")
      options["onclick"] ||= 'return true;'
      options["onclick"] = "var sel = document.getElementById('issue_category_id').selectedIndex; if(sel<1) {alert('#{confirm}'); return false;}"
    end
  ...
  #Funcion para campos no disponibles en bulk
  def custom_field_tag_for_bulk_edit_disabled(name, custom_field)
    field_name = "#{name}[custom_field_values][#{custom_field.id}]"
    field_id = "#{name}_custom_field_values_#{custom_field.id}"
    text_field_tag(field_name, '', :id => field_id, :disabled => true)
  end
  ...
  def custom_field_disabled_tag(prefix, custom_value)
    custom_value.custom_field.format.edit_tag self,
      custom_field_tag_id(prefix, custom_value.custom_field),
      custom_field_tag_name(prefix, custom_value.custom_field),
      custom_value,
  ...
  # Return custom field tag with its label tag
  def custom_field_tag_disabled_with_label(name, custom_value)
    custom_field_label_tag(name, custom_value) + custom_field_disabled_tag(name, custom_value)
  end

  def allowed_by_custom_field?(object, role_id, custom_field_id)
    cfur = object.find_by_role_id_and_custom_field_id(role_id, custom_field_id)
    cfur.blank? ? permission = false : permission = true 
    permission
  end

  def allowed_by_custom_field_status?(object, status_id, custom_field_id)
    cfur = object.find_by_issue_status_id_and_custom_field_id_and_is_required(status_id, custom_field_id,1)
    cfur.blank? ? permission = false : permission = true 
    permission
  end
