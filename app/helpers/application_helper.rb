...
module ApplicationHelper
  ...
  # @param [Hash] options Hash params. This will checked by authorize_for to see if the user is authorized, :permissions tiene un arreglo de permisos, :authorize_for es una bandera para saber si se va a utilizar el metodo authorize_for como validacion 
  # @param [optional, Hash] html_options Options passed to link_to
  # @param [optional, Hash] parameters_for_method_reference Extra parameters for link_to
  def link_to_if_authorized(name, options = {}, html_options = nil, *parameters_for_method_reference)
    if options[:permissions] && options[:permissions].is_a?(Array)
      permissions = options[:permissions]
      link = ""
      permissions.each { |permission|
        if User.current.allowed_to?(permission, @project)
          options.delete(:permissions)
          if options[:authorize_for] && options[:authorize_for] == false
            options.delete(:authorize_for)
            link = link_to(name, options, html_options, *parameters_for_method_reference)
          else
            link = link_to(name, options, html_options, *parameters_for_method_reference) if authorize_for(options[:controller] || params[:controller], options[:action])
          end
          break        
        end
      }
