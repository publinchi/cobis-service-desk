...
class Issue < ActiveRecord::Base
  ...
  def custom_obligatorios
    ActiveRecord::Base.connection.select_values("select custom_field_id from custom_fields_statuses where issue_status_id=#{self.status.id} and is_required=1")
  end

  def get_assignable_users
    members = Member.find(:all, :conditions => ["project_id=?", self.project_id], :select => "DISTINCT(id)")
    ...
    member_roles.each { |mr|      
      ...
      (member_roles_tmp << mr.role_id) if role.assignable == true
    }
    workflowrules = WorkflowRule.find(:all, :conditions => ["tracker_id=? and old_status_id=? and type='WorkflowTransition' and role_id IN (?)", self.tracker.id, self.status.id, member_roles_tmp.uniq], :select => "role_id")
    member_roles = MemberRole.find(:all, :conditions => ["role_id IN (?)", workflowrules.map(&:role_id).uniq], :select => "member_id")
    members = Member.find(:all, :conditions => ["id IN (?) and project_id=?", member_roles.map(&:member_id).uniq, self.project_id], :select => "user_id")
  ...
  def validate_issue
    ...
    #Validacion asignado a
    #1)Creacion con permiso para ver asignado
    errors.add(:assigned_to, :blank) if self.id.blank? && self.project.is_filtered==false && self.assigned_to.blank? && self.status.is_closed==false && self.project.asignado_a_obligatorio==true
    
    #2)Modificar issue
    errors.add(:assigned_to, :blank) if self.assigned_to.blank? && self.status.is_closed==false && self.project.asignado_a_obligatorio==true && !self.id.blank?

    #Validacion por etapas
    if !self.id.blank?
      @issue = Issue.find(self.id)
      @iss_status = IssueStatus.find(@issue.status_id) 
      @issue_antes_guardar1 = self.clone
      @issue_antes_guardar1.status = self.status
      @custom_values_antes_guardar1 = {}
      @custom_combo=[]
      @custom_combo = custom_obligatorios
      if User.current.allowed_to?(:view_private_notes, self.project)
        if (self.status.is_required_nota_publica_obligatoria==true && self.project.nota_publica_obligatoria_estado_cliente==true && self.status.id.to_i!= (Issue.find(self.id)).status_id)||((@iss_status.is_default == true && self.project.is_public_required == true && self.status_id.to_s != @iss_status.id.to_s))
          errors.add(:base, "Nota privada no debe estar seleccionada") if current_journal.private_notes == true 
      ...
      self.custom_values.each do |c|
        ...
        if User.current.allowed_to?(:view_private_notes, self.project)
          #Validacion nota publica de campo diagnostico
          @valor_fecha_diagnostico = custom_valor(self.id, 105)
          errors.add(:base, "Ingrese un texto en la nota.") if c.custom_field_id.to_i==107 && !c.value.blank? && @valor_fecha_diagnostico.blank? && @variable_nota_publica.blank? && current_journal.notes.blank?
        end

        #        @custom_combo.each do |tab|
        #          @custom=CustomField.find(c.custom_field_id)
        #          errors.add("#{@custom.name}", :blank) if tab.to_i==c.custom_field_id.to_i && (c.value=='0' or c.value=="") && @custom.is_required==false
        #        end
      end
      
      issue_status = IssueStatus.find(self.status_id)
      watchers = Watcher.find(:all, :conditions => ["watchable_id=?", self.id]) 
      errors.add(:base, "Seleccione seguidor(es).") if (issue_status.is_watcher_required == true && watchers.length == 0)
    end
  ...
  def validate_required_fields
    ...
    required_attribute_names(user).each do |attribute|
      if attribute =~ /^\d+$/
        ...
        elsif v && !v.value.blank? && v.custom_field_id == 58 && v.value == '0'
          errors.add :base, v.custom_field.name + ' debe ser Sí'
        end
      else
  ...
  def notified_users
    ...
    custom_values.each {|c|
      if current_user == autor
        @mail_sent_to_author = true
      else
        if c.custom_field.id == custom_field_id
          c.value == "1" ? @mail_sent_to_author = true :  (@mail_sent_to_author = false if c.value == "0")
        end
      end
    }
    ...
    #Si no se marca el checkbox ver notas publicas, el autor es quitado de la lista de correo
    notified.delete(author) if @mail_sent_to_author == false
    notified
  end
  ...
  # Returns the mail adresses of users that should be notified filtering watchers
  def recipients_filtered issue
    notified = project.notified_users
    # Author and assignee are always notified unless they have been
    # locked or don't want to be notified
    custom_values.each {|c|
    ...
    custom_values.each {|c|
      if c.custom_field.id == 59
        c.value=="1" ? @mail_sent_to_watcher=true : (@mail_sent_to_watcher=false if c.value == "0") 
      end
    }
    ...
    notified.reject! {|user| !visible?(user)}
    #Si no se marca el checkbox ver notas publicas, el autor es quitado de la lista de correo
    notified.delete(author) if @mail_sent_to_author==false && !author.allowed_to?(:view_private_notes, issue.project)
    
    notified.each { |i|
      notified.delete(i) if !i.allowed_to?(:view_private_notes, issue.project)
    } if @mail_sent_to_watcher==false
  ...
  def self.by_not_assigned_to(project)
    extra_count_and_group_by(:project => project,
      :field => 'assigned_to_id',
      :joins => User.table_name)
  end
  ...
  def excluded_rol_from_custom_field?(custom_field, au)
    validator=true
    roles=custom_field.roles
    users=au
    usuario_rol_ids = []
    users.each do |u|
      u.roles_for_project(@project).each do |ur|
        usuario_rol_ids << ur.id
      end if u.kind_of?(User)
    end
    roles.each do |r|
      validator=false if usuario_rol_ids.include? r.id
      return validator if validator == false
    end
    validator
  end
 
  # Returns an array of users that are proposed as routers
  def addable_router_users
    param = "%client_automatic_routing%"
    roles = Role.find(:all, :conditions => ["permissions like ?", param], :select => "id" )
    member_roles = MemberRole.find(:all, :conditions => ["role_id IN (?)", roles.uniq], :select => "DISTINCT(member_id)")
    members = Member.find(:all, :conditions => ["id IN (?) and project_id = ?", member_roles.map(&:member_id).uniq, self.project_id], :select => "DISTINCT(user_id)")
    users = User.find(:all, :conditions => ["id IN (?) and status!=?" , members.map(&:user_id).uniq, 3])
    add_group_to_user_list users if Setting.issue_group_assignment?
    users
  end
  ...
  def custom_valor(id, customid)
    ActiveRecord::Base.connection.select_value("select value from custom_values where customized_id=#{id} and custom_field_id=#{customid}")
  end
  ...
  def create_journal
    if @current_journal
      solucion_enviada_id = 58
      custom_values.each{|c|
        if c.custom_field.id == solucion_enviada_id
          @solucion_enviada = c.value
        end
      }
      
      solucion_enviada = @solucion_enviada
      # Hace que la nota pública sea siempre false si no soy cliente y si no he marcado el visto para que sea publica
      if User.current.client!=true
        visible = !@current_journal.private_notes
        #Valida que sea de ASP puede ejecutar sin problemas
        ...
        exec_sp_by_custom_field 23, @issue.id, visible, solucion_enviada
        exec_sp_by_custom_field 100, @issue.id, visible, solucion_enviada
        exec_sp_by_custom_field 105, @issue.id, visible, solucion_enviada
      end
      ...
      @current_journal.save
      #Validacion nueva fecha de primera respuesta version
      if @issue.has_first_answer_date.blank? and @variable_proyecto_tipo.blank? and !self.due_date.blank?
        if User.current.client!=true
          visible = !@current_journal.private_notes?
          exec_sp_by_custom_field 61, @issue.id, visible, solucion_enviada
        end
  ...
  def self.extra_count_and_group_by(options)
    project = options.delete(:project)
    select_field = options.delete(:field)

    where = "#{Issue.table_name}.#{select_field} is NULL"
    ...
    ActiveRecord::Base.connection.select_all("select    s.id as status_id,
                                              ...
                                              where 
                                                ...
                                                and #{Issue.table_name}.project_id=#{Project.table_name}.id
                                                and #{visible_condition(User.current, :project => project)}
                                              group by s.id, s.is_closed")
  end
  
  def exec_sp_by_custom_field custom_field_id, issue_id, visible, solucion
    config = Rails.configuration.database_configuration
    type = config[Rails.env]["type"]
    if type.blank?
      adapter = config[Rails.env]["adapter"]
    end
    custom_values.each {|c|
      if !c.custom_field.function.blank? && c.custom_field.id == custom_field_id && !solucion.blank?
        if type == "mysql" or (type.blank? && adapter == "mysql")
          connection.execute("call #{c.custom_field.function}(#{issue_id},'#{custom_field_id}',#{visible},#{solucion})");
        end
        if type == "postgresql" or (type.blank? && adapter == "postgresql")
          connection.execute("select #{c.custom_field.function}(#{issue_id},'#{custom_field_id}',#{visible},#{solucion})");
        end
      end
    }
  end
  ...
  def add_group_to_user_list users
    ...
    users_temp.each {|ut|
      users << ut if ut.is_a?(Group) and ut.status!=3
    }
  end
end
