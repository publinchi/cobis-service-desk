...
class IssueQuery < Query
  ...
  self.available_columns = [
    ...
    QueryColumn.new(:route_id, :sortable => "#{Issue.table_name}.route_id", :default_order => 'desc'),
    QueryColumn.new(:is_private, :sortable => "#{Issue.table_name}.is_private", :default_order => 'desc')
  ]
  ...
  def sql_for_watcher_id_field(field, operator, value)
    ...
    a = value.to_s    
    if a == "*" #comprobar si se realiza la consulta de peticiones sin seguidor , esta viene con el parametro *
      case operator
      when "="
        if value.any?
          "#{Issue.table_name}.id #{ operator == '=' ? 'NOT IN' : 'IN' } (SELECT #{db_table}.watchable_id FROM #{db_table})"
        else            
          sql = "1=0"
        end
      else "!"
        if value.any?
          "#{Issue.table_name}.id #{ operator == '=' ? 'NOT IN IN' : 'IN' } (SELECT #{db_table}.watchable_id FROM #{db_table})"
        else          
          sql = "1=1"
        end
      end
